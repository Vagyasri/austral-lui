...
.PHONY: help
help:
	@echo "make install: create a virtual environment and install the required packages"
...
install-asl install-austral-sci-layer install-austral-scientific-layer install-lilas-web-processing:
	@echo "Installing austral-scientific-layer"
	@git clone git@gitlab-ssh.univ-lille.fr:loa/agora/lilas-web-processing.git austral-scientific-layer

.PHONY: install-austral-data-sample
install-austral-data-sample:
	@echo "Installing austral-data-sample"
	@git clone git@gitlab-ssh.univ-lille.fr:loa/agora/austral-data-sample.git

.PHONY: venv austral-ui-env
venv austral-ui-env:
	@echo "Creating the virtual environment $(VENV_DIR) ..."
	@$(PYTHON) -m venv $(VENV_DIR) && . $(VENV_DIR)/bin/activate && $(PIP) install --upgrade pip && $(PIP) install -r requirements.txt
